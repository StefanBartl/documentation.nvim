...
return function(H)
  ...
  -- quickfix_items (R11 P2): the shape `:DocMap impact` puts in the list.
  ...
  local handle = require("documentation").install(base)
  ...
  -- `watch`, so a `:DocMap` earlier in the session left a non-watching handle
  ...
  local live = require("documentation").install({
    root = live_root,
    source = "lua/demo",
    lua_root = "lua",
    watch = true,
