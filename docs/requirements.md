...
| [lib.nvim](https://github.com/StefanBartl/lib.nvim) | required — `notify`, `fs.*`, `usercmd`, `map`, `debounce`, `autocmd`, `cross.uv.spawn_capture` |
...
| [ui.nvim](https://github.com/StefanBartl/ui.nvim) | `ui.kit`/`ui.contextmenu` back `:DocBrowse` (the editor-side map navigator) and its picker/context menu -- lazily required, so nothing on the generate/check path needs it |
