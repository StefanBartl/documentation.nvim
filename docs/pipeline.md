...
| `unreferenced-module` | info | Suppressed for a module the newest `runtime-analysis.loaded` snapshot says a real session had loaded — the aggregator case this check names in its own comment (`lib.nvim` reaches every submodule through a string map, so no literal `require` exists to find) settled by evidence instead of left as a standing false positive. Suppression only, never escalation, and silent without a snapshot, which is CI's normal state. Required by no other file in the tree. |
