...
[`docs/ROADMAP/MULTILANG.md`](../MULTILANG.md) (what each language costs, and why);
...
Target scope, as decided: all five languages `docs/ROADMAP/MULTILANG.md` costs out —
...
then does the next one start — the same reasoning `docs/ROADMAP/MULTILANG.md`
...
- [ ] **`Documentation.Node` grows an owning-scope concept.** Functions
  ...
  functions to an `impl` block, Go methods to a receiver type. `docs/ROADMAP/
...
- [ ] **`Documentation.Node` allows one file, many modules.** Rust's `mod x
  ...
  assumption `scan.lua`'s walk is built on. The second of `docs/ROADMAP/
...
- [ ] **Visibility as a first-class `Documentation.FunctionInfo` field.**
  ...
  capitalisation, TS has `private`. Third IR gap from `docs/ROADMAP/MULTILANG.md`.
...
Sequenced first per `docs/ROADMAP/MULTILANG.md`'s own conclusion: closest doc-tag
...
- [x] JSDoc/TSDoc doc-block parsing: `@param`, `@returns`, `@see`,
  ...
  partly redundant — `docs/ROADMAP/MULTILANG.md` already calls this a feature, not
...
- [ ] Docstring **style detection**: reST, Google, and NumPy conventions
  coexist across real Python codebases, sometimes within one. `docs/ROADMAP/
...
- [ ] rustdoc (`///`) is prose Markdown with **no tag vocabulary at all**.
  `docs/ROADMAP/MULTILANG.md` is explicit: the param-shaped checks
...
- [ ] godoc's *entire* checkable convention is "the comment starts with the
  identifier it documents." `docs/ROADMAP/MULTILANG.md` calls this the worst fit of
...
- [ ] Doxygen `\param`/`@param` where present, absent where not — `docs/ROADMAP/
