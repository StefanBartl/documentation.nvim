...
-- --------------------------------------------------------- build identity
--
-- A rolling release cannot say which one it is. The only tag is
-- `standalone-latest` and it moves, so "which engine produced this" had no
-- answer at all — the question every bug report starts with.
--
-- Generated here rather than committed: a version file in the source tree is
-- either hand-maintained (and wrong) or written by a hook (and wrong on
-- anyone's checkout who does not have the hook). Written into the staging
-- tree only, so `lua/` stays free of build output and a source run simply
-- finds no module — which `--capabilities` reports as absent rather than
-- inventing.
--
-- **`dirty` is not decoration.** A binary built from a modified working tree
-- carries a commit sha that does not describe what is inside it, and that is
-- worse than no sha: it sends someone reading a bug report to the wrong
-- diff. Saying so costs one `git status --porcelain`.

---Run a command and return its first line, or `nil` if it produced nothing.
---
---`io.popen` rather than `os.execute`: the output is the point. A machine
---without git, or a source tarball with no `.git`, yields `nil` here and the
---stamp records what it could rather than failing a release build over
---provenance metadata.
---@param cmd string
---@return string|nil
local function first_line(cmd)
  local out = io.popen(cmd .. " 2>" .. (package.config:sub(1, 1) == "\\" and "NUL" or "/dev/null"))
  if not out then
    return nil
  end
  local line = out:read("l")
  out:close()
...
local commit = first_line("git -C " .. quoted(repo) .. " rev-parse --short=12 HEAD")
local when = first_line("git -C " .. quoted(repo) .. " log -1 --format=%cI")
local status = first_line("git -C " .. quoted(repo) .. " status --porcelain --untracked-files=no")

local stamp = {
  "-- Generated by scripts/package.lua at bundle time. Not in version control:",
  "-- see that file's build-identity section for why a committed version file",
  "-- is either hand-maintained and wrong, or hook-written and wrong on",
  "-- somebody else's checkout.",
  "return {",
  ("  commit = %s,"):format(commit and ("%q"):format(commit) or "nil"),
  ("  committed_at = %s,"):format(when and ("%q"):format(when) or "nil"),
  -- `status` is nil when the tree is clean *and* when git could not answer.
  -- Those are different, and the difference is exactly what makes the sha
  -- trustworthy, so an unknown one is recorded as unknown.
  ("  dirty = %s,"):format(commit and tostring(status ~= nil) or "nil"),
  "}",
  "",
}

local stamp_rel = "documentation/build.lua"
local stamp_fd = assert(io.open(stage .. "/" .. stamp_rel, "wb"), "cannot write the build stamp")
stamp_fd:write(table.concat(stamp, "\n"))
stamp_fd:close()
staged[#staged + 1] = stamp_rel
print(("   build %s%s"):format(commit or "unknown", (status ~= nil) and " (dirty)" or ""))
