...
git clone --quiet --depth 1 https://github.com/UserNobody14/tree-sitter-dart.git "$work/tree-sitter-dart"
git clone --quiet --depth 1 https://github.com/tree-sitter/tree-sitter-scala.git "$work/tree-sitter-scala"
...
# Swift ships no generated parser, unlike every other grammar here, so this
# one grammar needs `generate` before `build`.
#
# Three things about this line, each of which cost a failed Windows run:
#
#   * **`"$TSC" generate`, not `npx tree-sitter generate`.** `npx` resolves
#     the CLI by its own rules, and *where npm puts a binary* has been a
#     moving target three times under MSYS2 already -- which is why `$TSC` is
#     pinned to a path this script chose.
#   * **`--ignore-scripts`.** The install exists so `grammar.js` can resolve
#     its own requires. It does *not* exist to compile this grammar's node
#     binding, which is what the postinstall does and what fails here: the
#     parser is generated by `$TSC` and built by `$TSC`, so a native node
#     module is work whose output nothing reads.
#   * **not `--silent`.** Two Windows runs failed on this exact command with
#     nothing whatsoever on stderr, which made "which step died" a guess and
#     cost a round trip each time. A quiet success is not worth a silent
#     failure.
echo "== swift: generate (the one grammar shipping no parser)"
(cd "$work/tree-sitter-swift"   && npm install --ignore-scripts --no-audit --no-fund   && "$TSC" generate)
...
"$TSC" build --output "$work/grammars/dart.$GSUF" "$work/tree-sitter-dart"
"$TSC" build --output "$work/grammars/scala.$GSUF" "$work/tree-sitter-scala"
