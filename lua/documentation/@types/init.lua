...
---@field generate_all? Documentation.GenerateAllOpts `usrcmds.setup()` only. Cross-repo project list for `:DocMap all`/`:DocMapAll` — plain data a consuming plugin's own spec supplies (this file never reads a config's plugin list itself). Absent or empty `projects`: neither action is registered at all. See `bindings/usrcmds/generate_all.lua`.

---`opts.generate_all` — the cross-repo project list `:DocMap all`/`:DocMapAll`
---and, when `autoload` is set, the first-load check both read.
