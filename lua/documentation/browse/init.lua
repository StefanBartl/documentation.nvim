...
local function load_impact(st, sha)
  ...
  local function ir_at(rev)
    ...
    return require("documentation.browse.source").rehydrate(doc)
  end

  local ir_after = ir_at(sha)
  local ir_before = ir_at(sha .. "^")
  st.impact_has_map = ir_after ~= nil
  st.impact = require("documentation.history").analyze(diff_text, ir_after, ir_before)
end
...
local function impact_to_quickfix(st)
  ...
  local hull, direct = require("documentation.deps").impact(st.ir, target)
  if #hull == 0 then
...
function M.open(opts)
  ...
  if opts.center and opts.center ~= "" then
    local found =
      require("documentation.command").find_node(ir, opts.center, opts.lua_root or "lua")
    if found then
      center = found
    else
