---@module 'documentation.core.luals'
...
--- than making every `:DocMap` slower. `scan_full()` in `init.lua` is what
...
---`doc.json`.
---
---Blocking (the CLI/`:DocMap full` callers need a result before continuing),
...
---string (e.g. `"table<string, Documentation.Node>"` -> `{"Documentation.Node"}`),
...
---dotted and namespaced (`Documentation.Node`, not `Node`), so collisions with
...
---`run()` was given (verified: scanning `lua/documentation` reports
...
function M.merge(ir, doc_json, source)
  ---@type table<string, Documentation.Node>
  local by_types_file = {}
