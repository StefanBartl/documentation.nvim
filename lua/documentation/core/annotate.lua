...
function M.apply(plan, root, mode)
  ...
  -- Re-verify against the current text immediately before writing: `plan`
  -- may have been computed many event-loop ticks ago (the chunked caller in
  -- `bindings/usrcmds/annotate.lua` spreads planning and applying across
  -- separate `vim.schedule` steps), and a save, formatter or checkout in
  -- between would otherwise be silently discarded by writing `plan.lines`
  -- wholesale. A mismatch means the plan is stale, so it is skipped rather
  -- than blindly applied.
  local read = require("lib.nvim.fs.read")
  local current = read(abs)
  if current == nil or current ~= table.concat(plan.lines, "\n") then
    return false, "file changed since it was scanned; skipped, rerun :DocMap annotate"
  end
