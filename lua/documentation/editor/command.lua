...
--- `:DocMap` — regenerate or verify the module map from inside Neovim.
---
---   :DocMap          regenerate the artifacts
---   :DocMap check    verify without writing (what the hook runs)
---   :DocMap full     regenerate with LuaLS enrichment (class/alias detail,
...
--- scanning a second time) and drives every action through it, so `:DocMap`
...
---
---Delegates to [`core/find.lua`](../core/find.lua), where it now lives: the
---lookup touches nothing but the IR, and `tagfiles.lua` needs it too — a core
---module reaching into the editor half for it is exactly what the
---`documentation.core` layer rule forbids. Kept here as an alias because it
---is part of this module's published surface.
...
function M.setup(opts)
  ...
  local cfg =
    require("documentation.core.config").build((opts and opts.root) or default_root(), opts)
  ...
  usercmd.create(command_name, function(args)
    ...
    -- :DocMap graph {deps|calls} [module]
    --
    ...
    -- and the working tree, `HEAD` by default. So bare `:DocMap impact` is
    ...
    --
    -- Not an Analysis panel, and it cannot become one: git data would make
    -- the committed artifact invalidate itself on the very commit that
    -- embedded it, since `--check` byte-compares. Same wall the History tab
    -- hit. So it lives here beside `impact` — live-computed into the quickfix
    -- list, nothing written.
    if action == "churn" or action:match("^churn%s") then
      local range = vim.trim(action:match("^churn%s*(.-)$") or "")
      local out_dir = cfg.out_dir or "docs/map"

      -- One record per commit: a record separator, then the paths it touched.
      -- `--no-merges` because a merge lists everything either side changed,
      -- which counts the same edit twice and rewards long-lived branches.
      -- Excluding the map is not cosmetic — in a repository that commits its
      -- own map it is regenerated by nearly every commit, so it would outrank
      -- everything real by an order of magnitude.
      ...
      if proc.code ~= 0 then
        notify.warn("git log failed: " .. vim.trim(proc.stderr or ""))
        return
      end
      ...
      for record in (proc.stdout or ""):gmatch("[^\30]+") do
        ...
        if next(touched) then
          ...
          -- Per commit, not per occurrence: a path listed twice in one commit
          -- is still one change to it.
      ...
      local result = churn.rank(counts, handle.ir(), seen_commits)
      ...
      vim.fn.setqflist({}, " ", {
        ...
        items = churn.quickfix_items(result, cfg.root),
      })
      vim.cmd("copen")
    ...
    if action == "dot" or action:match("^dot%s") then
      ...
      -- second `:DocMap dot` produced an unnamed buffer and no message at
    ...
    desc = "Regenerate the module map (:"
      .. command_name
      .. " [check|full|open|graph|why <a> <b>|dot|diff <ref>|impact <ref>|churn [range]|serve [stop]])",
    complete = function(lead, line)
      ...
      local candidates =
        { "check", "full", "open", "graph", "why", "dot", "diff", "impact", "churn", "serve" }
  ...
  -- :DocBrowse [live] [module] — the same map, navigated inside the editor.
  --
