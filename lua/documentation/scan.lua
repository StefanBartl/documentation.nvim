...
function M.scan(opts)
  ...
  local function walk_dir(abs, rel, parent_id, depth)
    ...
    if has_init then
      fns, calls, requires, syms, loc = require("documentation.functions").scan_file(init)
    end
    ...
    for _, e in ipairs(dir_entries) do
      ...
      elseif e.name:match("%.lua$") and e.name ~= "init.lua" then
        ...
        local leaf_fns, leaf_calls, leaf_requires, leaf_syms, leaf_loc =
          require("documentation.functions").scan_file(child_abs)
  ...
  require("documentation.deps").build(ir)
  require("documentation.calls").build(ir, opts)
