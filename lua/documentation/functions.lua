...
local function parse_doc_block(raw_lines)
  ...
  local summary = require("documentation.scan").split_summary(table.concat(prose, "\n"))
...
function M.scan_file(path)
  ...
  local requires = require("documentation.deps").extract_source(src)
  ...
  require("documentation.deps").mark_deferred(root, src, requires)

  local calls_mod = require("documentation.calls")
  ...
  local symbols = require("documentation.symbols").extract(root, src, doc_block_above)
